...
import type { DataTableFilterMeta, DataTableExpandedRows } from 'primevue/datatable'
...
const filters1 = ref<DataTableFilterMeta | undefined>(undefined)
const globalFilterValue = computed<string | null>({
  get: () => {
    const meta = filters1.value?.['global']
    if (meta && typeof meta === 'object' && 'value' in meta)
      return (meta.value as unknown as string | null) ?? null
    return null
  },
  set: (val) => {
    const meta = filters1.value?.['global']
    if (meta && typeof meta === 'object' && 'value' in meta) {
      ;(meta as unknown as { value: string | null }).value = val
    }
  },
})
const loading1 = ref<boolean>(true)
const balanceFrozen = ref<boolean>(false)
const products = ref<ProductWithOrders[]>([])
const expandedRows = ref<DataTableExpandedRows>({})
const statuses = reactive<string[]>([
  'unqualified',
  'qualified',
  'new',
  'negotiation',
...
const representatives = reactive<{ name: string; image: string }[]>([
...
function getOrderSeverity(order: { status: string }): string | undefined {
  switch (order.status) {
    case 'DELIVERED':
      return 'success'
...
function getStockSeverity(product: Product): string | undefined {
...
onBeforeMount(async () => {
  ...
  products.value = await productStore.fetchProductsWithOrdersSmall()

  const customersData = await customerStore.fetchCustomers()
  customers1.value = customersData.map((c) => ({
    ...c,
    date: new Date(c.date) as unknown as string,
  }))
  loading1.value = false
  customers2.value = await customerStore.fetchCustomers()
  customers3.value = await customerStore.fetchCustomers()

  initFilters1()
})

function initFilters1(): void {
  filters1.value = {
...
function expandAll(): void {
  expandedRows.value = products.value.reduce((acc: DataTableExpandedRows, p) => {
    acc[p.id] = true
    return acc
  }, {})
}
      ...
      <template #header>
        <div class="flex justify-between">
          ...
          <IconField>
            ...
            <InputText v-model="globalFilterValue" placeholder="Keyword Search" />
          </IconField>
        </div>
      </template>
