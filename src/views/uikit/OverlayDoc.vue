...
const visibleFull = ref<boolean>(false)
...
function toggleDataTable(event: MouseEvent): void {
  op.value?.toggle(event)
}

function onProductSelect(event: DataTableRowSelectEvent): void {
  op.value?.hide()
  toast.add({
    severity: 'info',
    summary: 'Product Selected',
    detail: (event.data as Product).name,
    life: 3000,
  })
}
