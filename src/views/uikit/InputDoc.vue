...
const selectedAutoValue = ref<Country | Country[] | null>(null)
const autoFilteredValue = ref<Country[]>([])
const calendarValue = ref<Date | null>(null)
const inputNumberValue = ref<number | undefined>(undefined)
const sliderValue = ref<number>(50)
const sliderInputValue = computed<string | null>({
  get: () => String(sliderValue.value),
  set: (v) => {
    sliderValue.value = v ? Number(v) : 0
  },
})
...
const listboxValues = ref<CityOption[]>([
...
const listboxValue = ref<CityOption | null>(null)
const dropdownValues = ref<CityOption[]>([
  { name: 'New York', code: 'NY' },
  { name: 'Rome', code: 'RM' },
  { name: 'London', code: 'LDN' },
  { name: 'Istanbul', code: 'IST' },
...
const dropdownValue = ref<CityOption | null>(null)
const multiselectValues = ref<CityOption[]>([
...
const multiselectValue = ref<CityOption[] | null>(null)
const toggleValue = ref<boolean>(false)
const selectButtonValue = ref<SelectOption | null>(null)
const selectButtonValues = ref<SelectOption[]>([
  { name: 'Option 1' },
  { name: 'Option 2' },
  { name: 'Option 3' },
])
const knobValue = ref<number>(50)
const inputGroupValue = ref<boolean>(false)
const treeSelectNodes = ref<TreeNode[]>([])
const selectedNode = ref<TreeNode | null>(null)

onMounted(async () => {
  autoValue.value = await countryStore.fetchCountries()
  treeSelectNodes.value = await nodeStore.fetchTreeNodes()
})

function searchCountry(event: AutoCompleteCompleteEvent): void {
  setTimeout(() => {
  ...
  <Fluid class="flex flex-col md:flex-row gap-8">
    <div class="md:w-1/2">
      ...
      <div class="card flex flex-col gap-4">
        ...
        <InputText v-model="sliderInputValue" />
